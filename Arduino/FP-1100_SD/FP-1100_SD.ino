...
//ファイル名の最後が「.fpt」でなければ付加
void addfpt(char *f_name,char *m_name){
  unsigned int lp1=0;
  while (f_name[lp1] != 0x00){
    m_name[lp1] = f_name[lp1];
    lp1++;
...
//状態コード送信(OK)
    snd1byte(0x00);
      ...
      g_adrs1 = rcv1byte();
      g_adrs2 = rcv1byte();
      g_adrs = g_adrs2*256+g_adrs1;
      file_w.write(g_adrs1);
      file_w.write(g_adrs2);
