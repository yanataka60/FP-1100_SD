...
　現在バージョンは修正済みですが、旧バージョンROMの方はZ80フォルダ内のPATCH.binをSDにコピーし、SDF8(SDE8)を読み込み実行直後にSDアクセスメニューに入り読み込み実行すれば修正されます。
...
2025.8.12 FP-1100_SD以外のPACKを同時に使うときにはSLOTを再指定する必要があったので修正。
