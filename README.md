...
　SDアクセスルーチンをF800h～FFE9hに読み込む「SDF8」とE800h～EFE9hに読み込む「SDE8」の２種類を用意していますのでどちらか好きなようを使います。

　「SDE8」を使うときは、「RUN"PACK0:SDF8"」を実行します。

　F000h以降に機械語を読み込みたい以外は「SDF8」を使うことを想定しています。

　SDアクセスメニュー表示するにはCALL&HF800(CALL&HE800)を実行しますが、F7ファンクションキーに登録してありますのでF7ファンクションキーを押しても大丈夫です。

#### 扱えるファイル
bugfire2009さんのDumpListEditorを使ってバイナリ形式に変換したものを使います。

　機械語 -> 保存は「各モデル固有ヘッダ付きBINファイル」、拡張子は.bin

![DumpListEditor1](https://github.com/yanataka60/FP-1100_SD/blob/main/JPEG/DumpListEditor1.jpg)

　BASIC -> 保存は「BASファイル」、拡張子は.bas

![DumpListEditor2](https://github.com/yanataka60/FP-1100_SD/blob/main/JPEG/DumpListEditor2.jpg)
...
#### SL[RETURN]
　入力待ちになるので0又は1キーでSLOT番号を入力します。0,1以外の場合は再度入力待ちになります。
